...
def get_filtered(df_use,focus, minhr,mincount,minHPC):
    df_copy=df_use.copy()
    
    filt01=(df_copy[focus]>=minhr)&(df_copy['HPC']>=minHPC)
...
def get_items(df, focus, minval,minHPC):
    cleancodes=df.clean
    strvals=""
    count=0
    extension=", >>  "+focus+" >= "+str(minval)+" minHPC="+str(minHPC)
    for c in cleancodes:
        strvals+=c+","
        count+=1
        if count==8:
...
def program34(focus,minhr,mincount,raindoc,minHPC,listlimit):
    ...
    df_out1=get_filtered( df_main,focus,minhr,mincount,minHPC )
    items=get_items(df_out1,focus,minhr,minHPC)

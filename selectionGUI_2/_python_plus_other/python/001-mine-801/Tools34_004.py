...
def get_filtered(df_use,focus, minhr1,minhr2,minHPC,minCh,mincount):
    df_copy=df_use.copy()
    
    filt01=(df_copy[focus]>=minhr1)&(df_copy['5-H']>=minhr2)&\
    (df_copy['HPC']>=minHPC)&(df_copy['Change']>=minCh)
    
    df_filtered=df_copy.loc[filt01]
    
    df_result=df_filtered.groupby(['Code'])[['SR']]\
...
def get_items(df, focus, minHR1,minhr2,minHPC,minCh):
    cleancodes=df.clean
    strvals=""
    count=0
    extension=", >>  "+focus+" >= "+str(minHR1)+" 5-H >="+str(minhr2)
...
def program34(focus,minhr1,minhr2,minHPC,minCh,mincount,raindoc,listlimit,createfiles):
    
    raindoc.insert_line("")
    title="focus="+focus+" minhr="+str(minhr1)+" minhr2="+str(minhr2)
    ...
    df_out1=get_filtered( df_main,focus,minhr1,minhr2,minHPC,minCh,mincount )
    items=get_items(df_out1,focus,minhr1,minhr2,minHPC,minCh)
    ...
    for i in range(0,len(codes)):
        ...
        strv=width(codes[i],9,char="_")+"\t ->"+\
             width(counts[i],3)+"\t"+\
             f2(means[i])+"\t"+f2(meds[i])+"\t"+f2(mins[i])+ast 

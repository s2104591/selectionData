...
# Jan14, 1353, changed sort to variable input ie min, mean, or count
# Jan15, 1653 , changed sort to accept median sort also
# Jan28, 10:31am, 
...
def getsort(sort):
    if sort=='mean' or sort=='min' or sort=='count' or sort=='median':
        return sort
    return 'mean'

def get_filtered(df_use,minhr1,minhr2,minHPC,minCh,mincount,minavg,sort,minmin):
    df_copy=df_use.copy()
...
def program34(minhr1,minhr2,minHPC,minCh,mincount,raindoc,listlimit,createfiles,\
              minavg,sort='mean',minmin=0):
    ...
    df_out1=get_filtered( df_main,minhr1,minhr2,minHPC,minCh,mincount,minavg,sort,minmin )
    ...
    for i in range(0,len(codes)):
        ...
        strv=width(codes[i]+"-",7,char="_")+"\t ->"+\
             width(counts[i],3)+"\t"+\
             f2(means[i])+"\t"+f2(meds[i])+"\t"+f2(mins[i])+ast +"\t hereJan15"+\
...
def get_MTdict(cc,full):
    ...
    for i in range(length):
        ...
        b=full[i]
        result[a]=b+"$"
        #print("get_MTdict-> dictionary add",a,b)

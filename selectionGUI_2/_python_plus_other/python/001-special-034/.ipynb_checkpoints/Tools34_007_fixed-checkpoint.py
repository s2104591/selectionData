...
def get_filtered(df_use,minhr1,minhr2,minHPC,minCh,mincount,minavg):
    df_copy=df_use.copy()
...
def program34(minhr1,minhr2,minHPC,minCh,mincount,raindoc,listlimit,createfiles,minavg):
    ...
    duplicated=get_duplicated(df_main) # jan05  A




    df_out1=get_filtered( df_main,minhr1,minhr2,minHPC,minCh,mincount,minavg )
    ...
    for i in range(0,len(codes)):
        ...
        #countsItems.update([clean[i]])
        
        ast=""        
        if clean[i] in selected:
            ast="  *"

        strv=width(codes[i]+"-",5,char="_")+"\t ->"+\
             width(counts[i],3)+"\t"+\
             f2(means[i])+"\t"+f2(meds[i])+"\t"+f2(mins[i])+ast +"\t hereDec22"+\
             "\t"+dictMT.get(clean[i], "???->"+clean[i]) 
             #"\t"+get_duplicatedcomment(clean[i],duplicated) # Jan05, B
    ...
    #duplicates=[ code for code,count in countsItems.most_common() if count>=2  ]
    #return duplicates
    return duplicated
